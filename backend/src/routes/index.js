...
router.put("/users/:id", auth, async (req, res) => {
  ...
  // Только admin может менять роль через общий PUT
...
router.patch(
  ...
  async (req, res) => {
    const { role } = req.body;
    if (!["admin", "manager", "executor"].includes(role)) {
...
router.get(
  ...
  auth,
  requireRole("admin", "manager"),
  exportController.exportClientsExcel,
);
router.get(
  "/export/invoices/:id/pdf",
  auth,
  requireRole("admin", "manager"),
...
router.get(
  ...
  auth,
  requireRole("admin", "manager"),
  exportController.exportReportExcel,
);
router.get(
  "/export/reports/pdf",
  auth,
  requireRole("admin", "manager"),
  exportController.exportReportPdf,
);
...
router.get("/google/auth", auth, (req, res) => {
  ...
  if (!url) {
    return res
      ...
      .json({
        error:
          "Google Calendar не настроен. Добавьте GOOGLE_CLIENT_ID и GOOGLE_CLIENT_SECRET в .env",
      });
  }
  res.json({ url });
});
